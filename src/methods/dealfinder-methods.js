...
function getRoutePicker(leg, route) {
    if (leg === 'return') {
        return Selector('.routes_return').find(
            `li[data-routenameret="${route}"]`
        );
    }
    return Selector('.routes_outbound').find(`li[data-routename="${route}"]`);
}
...
module.exports = {
    selectRoute: async function (leg, route) {
        ...
        await t
            .expect(Selector('.route_list h5').exists)
            .ok('Routes drop down not loaded', { timeout: 10000 })
            .expect(getRoutePicker(leg, route).exists)
            .ok('Correct route not loaded', { timeout: 10000 })
            .click(getRoutePicker(leg, route), { speed: 0.01 }); //slow down click to ensure it is registered
    },
